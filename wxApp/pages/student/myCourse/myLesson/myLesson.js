  ...
  semesterChange(e) {
    ...
    if (wx.getStorageSync("sign") == 1){
      if (this.data.semesterArr.length > 0) {
        ...
        appInstance.ajax("/courseTableChangeClass/getListByTermIdAndstundetId", { termId: this.data.terms[e.detail.value].id, studentId: this.data.studentId }, "get", (res) => {
          if (res.data.code == 1) {
            this.setData({ record: res.data.data, index: e.detail.value })
          } else {
            console.log('数据出错')
          }
      ...
      if (this.data.semesterArr.length > 0) {
        appInstance.ajax("/courseTableChangeClassForTeacher/getListByTeacherIdAndtermId", { teacherId: this.data.teacherId, termId: this.data.terms[e.detail.value].id }, "get", (res) => {
