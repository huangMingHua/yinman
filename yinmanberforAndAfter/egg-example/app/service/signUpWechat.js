    ...
    class signUpWechat extends app.Service { *
        add(courseTableDetailStudentId) {
            ...
            getListByIsSend(isSend) {
                let list = yield app.mysql.select('sign_up_wetchat', { where: { isSend: isSend } })
                if (isSend === 0) {
                    for (let item of list) {
                        let courseTableDetailStudent = yield this.ctx.service.courseTableDetailStudent.getById(item.courseTableDetailStudentId)
                        ...
                        for (let i of aCourseTableItem) {
                            let courseTableItem = yield this.ctx.service.courseTableItemStudent.getStudentByCourseTableItemIdAndStudentId(i.id, courseTableDetailStudent.studentId)
                ...
                if (isSend !== 0) {
                    for (let item of list) {
                        ...
                        let aCourseTableItem = yield this.ctx.service.courseTableItem.getByCourseTableDetailId(item.courseTableDetailStudent.courseTableDetailId)
