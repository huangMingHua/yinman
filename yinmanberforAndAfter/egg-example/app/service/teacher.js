module.exports = app => {
  class Teacher extends app.Service {
    ...
    *
      ...
      getList(page, limit) {
              const teachers = yield app.mysql.select('teacher', {limit:Number(limit), offset: Number(page - 1 )*limit});
              console.log(page, limit)        
      let totalCount = yield app.mysql.query('select count(*) from teacher');
              totalCount = totalCount[0]['count(*)'];
              const list = [];
              for (const item of teachers) {
                const user = yield this.ctx.service.user.getById(item.userId);
                list.push({ teacher: item, user });
              }
              const result = {
                list,
                totalCount,
    ...
    *
        getByUserId(userId) {
          const result = yield app.mysql.get('teacher', { userId });
          return result;
        }
