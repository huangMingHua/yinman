module.exports = app => {
    class Course extends app.Service { *
        getAll(page) {
                ...
                const result = yield app.mysql.select('course');
                return result;
            } *
            getPaging(page, limit) {
                console.log(page, limit);
                const list = yield app.mysql.select('course', { limit: Number(limit), offset: (page - 1) * limit });
            ...
            add(body) {
                const result = yield app.mysql.insert('course', { name: body.name, courseDescription: body.courseDescription });
                return result;
            } *
            modify(body) {
            ...
            getByName(name) {
                const info = yield app.mysql.get('course', { name: name });
                return info;
            }
    }
    return Course;
