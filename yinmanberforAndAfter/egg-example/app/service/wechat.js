        ...
        bookingSuccess(studentName, courseName, now, publicOpenId, studentId) {
            ...
            if (publicOpenId) {
                template("NI1hJJe3VGtw68_rDBTuQyY6m1aPWBg08ejAhSEcK6Y", {
                    ...
                    "pagepath": `pages/index/subscribeList/subscribeList?_id=${studentId}`
                }, data, publicOpenId)
            }
...
注意事项：待定。查看课程详情！`,
            ...
            sendBerforOneDayForbooking() {
                let list = yield this.ctx.service.bookingWechat.getListByIsSend()
                for (let item of list) {
                    console.log(111)
                    let sendTime = new Date(item.sendTime)
                    let now = new Date()
                    if (now.getTime() >= sendTime) {
                        let bookingCourse = yield this.ctx.service.bookingCourse.getById(item.bookingCourseId)
                        if (!bookingCourse) {
                            continue
                        }
                        let courseTableItem
                        ...
                        let student = yield this.ctx.service.student.getById(bookingCourse.studentId)
            ...
            signUpSuccess(studentName, courseName, classTime, publicOpenId, studentId) {
                ...
                if (publicOpenId) {
                    template("oYN1SQt6mPcH5ZTfHX_-o653WtOxBmsusIBGRVO3gII", {
                        "appid": "wxdc06b91aef63b7b4",
                        "pagepath": `pages/index/signUpList/signUpList?_id=${studentId}`
                    }, data, publicOpenId)
                }
            } *
            ...
            sendForSignUp() {
                let list1 = yield this.ctx.service.signUpWechat.getListByIsSend(1)
                let list2 = yield this.ctx.service.signUpWechat.getListByIsSend(2)
                let list3 = yield this.ctx.service.signUpWechat.getListByIsSend(3)
                yield sendSignUp(this, list1, 30, 2)
                yield sendSignUp(this, list2, 15, 3)
                yield sendSignUp(this, list3, 1, 4)

                function* sendSignUp(This, list1, day, num) {
                    for (let item of list1) {
                        let now = new Date()
                        let date = new Date(app.moment(item.courseTableItem.date).subtract(day, 'days'))
                        let day1
                        if (day === 30) {
                            day1 = 15
                        }
                        ...
                        let date1 = new Date(app.moment(item.courseTableItem.date).subtract(day1, 'days'))
                        if (now.getTime() >= date.getTime() && now.getTime() <= date1.getTime()) {
                            let student = yield This.ctx.service.student.getById(item.courseTableDetailStudent.studentId)
                            let user = yield This.ctx.service.user.getById(student.userId)
                            let courseTableDetail = yield This.ctx.service.courseTableDetail.getById(item.courseTableDetailStudent.courseTableDetailId)
                            let classTime = app.moment(item.courseTableItem.date).format('YYYY.MM.DD')
                            let data = {
                                "userName": {
                                    "value": `${student.name}的家长！${student.name}的课程还有${day}天结束：`,
                                    "color": "#173177"
                                },
                                "courseName": {
                            ...
                            template("0Hu_Bk-ecwGXshpOvBpTL6zpoV_yZPE07JLeZ9s3GeI", {
                                "appid": "wxdc06b91aef63b7b4",
                                "pagepath": `pages/index/signUpList/signUpCourse/signUpCourse?studentId=${student.id}`
                            }, data, user.publicOpenId)
                            let signUpWechat = {
                                id: item.id,
                                isSend: num,
            ...
            sendForSignUp1() {
                let list0 = yield this.ctx.service.signUpWechat.getListByIsSend(0)
...
请按补课时间准时上课，谢谢！查看补课记录！`,
                ...
                let newClassTime1 = newClassTime.split(' ')
                let data1 = {
                    "first": {
            ...
            addStudentLeave(studentName, courseName, classTime, teacherName, reason, publicOpenId, teacherPublicOpenId, studentId) {
                let data = {
                    "first": {
                ...
                if (publicOpenId) {
                    template("pvw9gahMaaGSFknb0LY8QQyxZPc10uenS0uLQUIJqhM", {
                        "appid": "wxdc06b91aef63b7b4",
                        "pagepath": `pages/student/myCourse/myLeave/myLeave?studentId=${studentId}`
                    }, data, publicOpenId)
                }
                let data1 = {
                    "first": {
            ...
            addMakeUp(studentName, classDate, classTime, teacherName, classroom, publicOpenId, teacherPublicOpenId, studentId, courseName) {
