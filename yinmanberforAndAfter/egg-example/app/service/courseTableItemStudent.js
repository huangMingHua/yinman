  ...
  class courseTableItemStudent extends app.Service {
    get db() {
      return this.ctx.conn ? this.ctx.conn : this.app.mysql;
            ...
            getCourseTableDetailStudentByCouseTableDetailIdAndStudentId(courseTableDetailId, studentId) {
              const result = yield app.mysql.select('course_table_detail_student', { where: { courseTableDetailId, studentId } });
              return result;
            }
            /**
    ...
    *
        getLengthByCourseTableItemId(courseTableItemId) {
          const list = yield app.mysql.query(`SELECT * FROM course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND (status='补课' OR status='正常') `);
          return list.length;
        }
    ...
    *
        getLengthByCourseTableItemIdAndStudentId(courseTableItemId, studentId) {
          const list = yield app.mysql.query(`SELECT * FROM course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND studentId=${studentId} AND (status='补课' OR status='正常' OR status='停课' OR status='转课') `);
          return list.length;
        }
        *
        getListByCourseTableItemIdAndStudentId(courseTableItemId, studentId) {
          const list = yield app.mysql.query(`SELECT * FROM course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND studentId=${studentId} AND (status='补课' OR status='正常' OR status='停课' OR status='转课') `);
          return list;
        }
        /**
    ...
    *
        getByCourseTableItemIdAndStudentIdAndStatus(courseTableItemId, studentId, status) {
          const get = yield app.mysql.query(`select * FROM course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND studentId=${studentId} AND  status='${status}'`);
          return get[0];
        }
        /**
         * 某节课里面有没有当前学生（条件不一样）
    ...
    *
        getLengthByCourseTableItemIdAndStudentId1(courseTableItemId, studentId) {
          const list = yield app.mysql.query(`SELECT * FROM course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND studentId=${studentId} AND status!='请假'`);
          return list.length;
        } *
        ...
        getByCourseTableItemIdAndStudentId1(courseTableItemId, studentId) {
          const list = yield app.mysql.query(`SELECT * FROM course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND studentId=${studentId} AND status='正常'`);
          return list;
    }
    ...
    *
        autoUpdateStatus(courseTableItemId) {
          ...
          const toCourseTableItemStudents = yield this.service.courseTableItemStudent.getByCourseTableItemId(courseTableItemId, '正常');
          const toCourseTableItemStudents1 = yield this.service.courseTableItemStudent.getByCourseTableItemId(courseTableItemId, '补课');
    ...
    *
        getLevelByCourseTableItemIdAndStudentId(courseTableItemId, studentId) {
          const result = yield app.mysql.get('course_table_item_student', { courseTableItemId, studentId, status: '请假' });
          return result;
        } *
        update(info) {
        ...
        delete(courseTableItemId, studentId) {
            ...
            //     studentId,
            // });
          const result = app.mysql.query(`DELETE FROM  course_table_item_student WHERE courseTableItemId=${courseTableItemId} AND studentId=${studentId} AND status!='请假'`);
          yield this.autoUpdateStatus(courseTableItemId);
        }
        *
        getById(id) {
          const result = app.mysql.get('course_table_item_student', {id});
          return result
        }
    }
