    ...
    WechatController.prototype.wechat = wechat(
        ...
        if (message.Event === 'subscribe') {
            for (let i = 0; i < getListUser.res.data.data.openid.length; i++) {
                let getUnionid = yield app.curl(`https://api.weixin.qq.com/cgi-bin/user/info?access_token=${getAccessToken.data.access_token}&openid=${getListUser.res.data.data.openid[i]}&lang=zh_CN`, {
                    dataType: 'json',
                });
                let user = yield this.service.user.getUnionid(getUnionid.data.unionid);
                console.log('关注', user)
                if (user) {
                    console.log('关注1')
                    ...
                    console.log('关注2')
                    let id = yield this.service.user.add(getUnionid.data.headimgurl, getUnionid.data.nickname, "", 1, "", getUnionid.data.sex, 1, app.moment().format('YYYY-MM-DD HH:mm:ss'), app.moment().format('YYYY-MM-DD HH:mm:ss'), getUnionid.data.unionid, getListUser.res.data.data.openid[i]);
                    this.ctx.body = '关注成功';
                }
            }
        }
