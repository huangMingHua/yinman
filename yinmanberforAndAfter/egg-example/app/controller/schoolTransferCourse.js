module.exports = app => {
    ...
    class schoolTransferCourse extends app.Controller { *
        createCourseInfo() {
                ...
                let iTermDate = yield this.ctx.service.term.getById(body.nTermId)
                ...
                let aDate = yield this.ctx.service.courseTableItem.getAll()
                let aId = []
                let aEnd = []
                for (let item of aDate) {
                ...
                let result = yield app.mysql.beginTransactionScope(function*(conn) {
                    for (var i = 0; i < aId.length; i++) {
                        let data = yield This.ctx.service.courseTableItem.findById(aId[i])
                        data.remarks = {
                        ...
                        let oldTime = app.moment(data.date).format('YYYY.MM.DD') + " " + data.startTime + "-" + data.endTime
                        let nowTime = app.moment(body.iMakeupTime).format('YYYY.MM.DD') + " " + data.startTime + "-" + data.endTime
                        data.date = body.iMakeupTime
                        yield app.mysql.update('course_table_item', data)
                        let courseTableItemStudentS = yield This.ctx.service.courseTableItemStudent.getByCourseTableItemIdNoleave1(aId[i])
                        let teacher = yield This.ctx.service.teacher.getById(data.teacherId)
                    ...
                    let result = yield This.ctx.service.schoolTransferCourse.createCourseInfo(body.nTermId, body.iClassSchedulingTime, body.iMakeupTime, body.iReason)
                }, This);
