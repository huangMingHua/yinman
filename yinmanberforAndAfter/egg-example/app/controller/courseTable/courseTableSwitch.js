  ...
  class CourseTableSwitchController extends app.Controller {
    ...
    *
        addSwitch() {
          ...
          const toCourseTableItemId = this.ctx.request.body.toCourseTableItemId || 0;
          ...
          const termId = fromCourseTableItem.termId;
          if (teacherId > 0) { // 老师
            const result = yield app.mysql.beginTransactionScope(function* (conn) {
              ...
              if (fromCourseTableItem.courseTableDetailId > 0) {
                ...
                const students = yield ctx.service.courseTableItemStudent.getByCourseTableItemId(fromCourseTableItem.id, '正常');
                if (students.length > 1) {
                  return { code: 0, msg: '当前课程有多名学生，无法调课' };
                }
                const originSwitch = yield ctx.service.courseTableItemSwitch.get(fromCourseTableItem.id, students[0].id);
              ...
              const switchMaxCount = parseInt((yield ctx.service.config.getByKey('教师调课次数')).value);
              const switchs = yield ctx.service.courseTableItemSwitch.getTeacherSwitchs1(courseTableDetailId, teacherId);
              ...
              const fromCourseTableItemStudents = yield ctx.service.courseTableItemStudent.getByCourseTableItemId(fromCourseTableItem.id);
          ...
          const result = yield app.mysql.beginTransactionScope(function* (conn) {
            ...
            if (fromCourseTableItem.courseTableDetailId > 0) {
              ...
              const originSwitch = yield ctx.service.courseTableItemSwitch.get(fromCourseTableItem.id, studentId);
              if (originSwitch == null) {
                                // this.fail('未找到源请假信息');
                                // return;
                return { code: 0, msg: '未找到源请假信息' };
            ...
            const switchMaxCount = parseInt((yield ctx.service.config.getByKey('学生调课次数')).value);
            const switchs = yield ctx.service.courseTableItemSwitch.getStudentSwitchs1(courseTableDetailId, studentId);
            ...
            yield ctx.service.courseTableItem.update(fromCourseTableItem);
            const fromCourseTableItemStudents = yield ctx.service.courseTableItemStudent.getByCourseTableItemIdAndStudentId(fromCourseTableItem.id, studentId, '正常');
            for (let i = 0; i < fromCourseTableItemStudents.length; i++) {
              const item = fromCourseTableItemStudents[i];
              item.status = '请假';
              yield ctx.service.courseTableItemStudent.update(item);
    ...
    *
            getSwitchDataForStudent() {
              ...
              for (const i in groupList) {
                const teacher1 = yield this.ctx.service.teacher.findById(parseInt(i));
                ...
                for (const l of teacher1.courseTableItems) {
    ...
    *
        getSwitchDetail() {
          const id = this.ctx.request.query.id || 0;
          const info = yield this.ctx.service.courseTableItemSwitch.getById(id);
          if (info == null) {
    ...
    *
        getList() {
          ...
          const data = yield this.service.courseTableItemSwitch.getList(pageIndex, termId, limit);
          for (const item of data.list) {
            ...
            const fromCourseTableItem = yield this.service.courseTableItem.getById(item.fromCourseTableItemId);
            ...
            const toCourseTableItem = yield this.service.courseTableItem.getById(item.toCourseTableItemId);
