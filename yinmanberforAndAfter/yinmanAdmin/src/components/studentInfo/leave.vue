      ...
      <thead>
        ...
        <tbody>
          <tr v-for="item in tableData">
            ...
            <td>{{item.courseTableItem.date}} {{item.courseTableItem.dateOfWeek}} {{item.courseTableItem.startTime}}~{{item.courseTableItem.endTime}}</td>
            <td>{{item.oClassroom.name}}</td>
            <td>{{item.level}}</td>
            <td>{{item.number}}</td>
            <td>{{item.reason}}</td>
            <td>{{item.createTime | viewFullDate}}</td>
          </tr>
        </tbody>
    </table>
  </div>
...
export default {
  ...
  methods:{
      ...
      getSwitch(){
       this.$http.get("/api/courseTableLeave/getListBystudentIdAndTermId",{params:{studentId: this.studentId,termId: this.termId}}).then(res=>{
            // console.log(res)
                  if(res.data.code==1){
                    this.tableData=res.data.data
                    for(let item of this.tableData){
                        let aDay = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六']
                      item.courseTableItem.dateOfWeek=aDay[new Date(item.courseTableItem.date).getDay()]
                    }
                  }
              })
      },
