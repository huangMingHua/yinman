    ...
    <div class="box">
        ...
        <div class="oldCourseDetail">
            ...
            <div class="courseDetail">
                ...
                <div>
                    <span>周期：</span>
                    <strong>{{courseTableDetail.startDate+'~'+courseTableDetail.endDate+' '+courseTableDetail.startTime+'~'+courseTableDetail.endTime}}</strong>
                </div>
                <div>
                    <span>停课时间：</span>
                    <strong>{{suspendClass.date+' '+suspendClass.startTime}}</strong>
                </div>
                <div>
                    <span>满员人数：</span>
        ...
        <div class="applyMakeUp">
            ...
            <div class="makeUpDetail">
                ...
                <div class="row">
                    <span>上课时间：</span>
                    <el-select v-model="courseTableItemId" placeholder="请选择">
                        <el-option
    ...
    export default{
        ...
        methods: {
            init () {
                ...
                this.$http.get('/api/courseTable/getCourseTableDetailStudentByCourseTabledetailIdAndStudentId',{params:{courseTableDetailId:this.courseTableDetailId,studentId:this.studentId}}).then((response)=>{
                    let res = response.data;
                    if (res.code) {
                        let {teacher,student,courseTableDetail,suspendClass,courseTableDetailStudent,classroom,course} = res.data;
                        this.student = student;
                        this.teacher = teacher;
                        this.courseTableDetail = courseTableDetail;
                        this.classroom = classroom;
                        this.course = course;
                        this.suspendClass = suspendClass;
            ...
            submit () {
                ...
                if (!this.courseTableItemId) {
                    return this.$message('请选择上课时间');
                }
                if (!this.reason) {
                    return this.$message('请输入转班原因');
                }
                this.$http.post('/api/courseTable/classTransfer',{courseTableDetailId:this.courseTableDetailId,classTransferCourseTableDetailId:this.courseTableDetailId1,studentId:this.studentId,shiftCourseTableItemId : this.courseTableItemId,reason:this.reason}).then((response)=>{
                    let res = response.data;
                    if (res.code) {
                        this.$emit('cancelClassTransfer');
                    } else {
